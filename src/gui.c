...
/* Virtual Resolution */
static const int V_WIDTH = 1280;
static const int V_HEIGHT = 720;
static float g_camera_scale = 1.0f;
static Vector2 g_camera_offset = {0};
static Font g_custom_font = {0};

/* ========== Yardımcılar ========== */
static void BeginScissorModeScaled(int x, int y, int width, int height) {
    int sx = (int)(g_camera_offset.x + x * g_camera_scale);
    int sy = (int)(g_camera_offset.y + y * g_camera_scale);
    int sw = (int)(width * g_camera_scale);
    int sh = (int)(height * g_camera_scale);
...
static void DrawTextC(const char *text, int x, int y, int size, Color color) {
    if (g_custom_font.texture.id > 0) {
        DrawTextEx(g_custom_font, text, (Vector2){(float)x, (float)y}, (float)size, 1.0f, color);
    } else {
...
static void draw_custom_scrollbar(float x, float y, float w, float view_h, float content_h, float *scroll) {
    ...
    DrawRectangleRounded((Rectangle){x, thumb_y, w, thumb}, 0.5f, 4, COLOR_SCROLLBAR);
}
...
static void draw_header(int W) {
    ...
    for (int i = 0; i < W; i++) {
        float t = (float)i / W;
        Color c = {(unsigned char)(99*(1-t)+139*t), (unsigned char)(102*(1-t)+92*t), (unsigned char)(241*(1-t)+246*t), 255};
...
static void draw_tabs(int W) {
    ...
    for (int i = 0; i < (int)TAB_COUNT; i++) {
        ...
        if (i == (int)g_active_tab) {
            ...
            if (hover) DrawRectangleRounded(btn, 0.4f, 6, (Color){255,255,255,6});
            DrawTextC(labels[i], tx+14, y+9, 13, hover ? COLOR_TEXT : COLOR_TEXT_SEC);
        }
        if (hover && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            g_active_tab = i;
            if (i == TAB_DASHBOARD) { g_selected_device_ip[0] = '\0'; g_scroll_devices = 0; g_selected_packet_num = -1; }
        }
        tx += tw + 4;
    }
}
...
/* ========== Stat Card ========== */
static void draw_stat_card(Rectangle r, const char *label, const char *value, Color valColor, const char *sub) {
    ...
    if (sub && sub[0]) DrawTextC(sub, r.x+12, r.y+48, 10, COLOR_TEXT_DIM);
}
...
static void draw_panel_dashboard(int W, int H) {
    ...
    snprintf(sub, sizeof(sub), "%s", g_scan.network_range[0] ? g_scan.network_range : "...");
    draw_stat_card((Rectangle){12, y0, cw-4, 60}, "Cihaz", buf, COLOR_ACCENT, sub);

    snprintf(buf, sizeof(buf), "%s", g_scan.gateway_ip[0] ? g_scan.gateway_ip : "...");
    draw_stat_card((Rectangle){12+cw, y0, cw-4, 60}, "Gateway", buf, COLOR_GREEN, NULL);

    snprintf(buf, sizeof(buf), "%s", g_scan.local_ip[0] ? g_scan.local_ip : "...");
    draw_stat_card((Rectangle){12+cw*2, y0, cw-4, 60}, "Bu Cihaz", buf, COLOR_CYAN, NULL);
    ...
    /* Sol panel */
    DrawRoundedPanel((Rectangle){12, list_y, list_w, list_h}, COLOR_PANEL, COLOR_BORDER);
    DrawTextC("Agdaki Cihazlar", 24, list_y+8, 12, COLOR_ACCENT);

    /* Tarama badge */
    {
        DrawCircle(12+list_w-14, list_y+14, 3, g_scan.is_scanning ? COLOR_AMBER : COLOR_GREEN);
    }
    ...
    if (CheckCollisionPointRec(GetMousePosition(), list_area)) {
        ...
        if (g_scroll_devices < 0) g_scroll_devices = 0;
        if (g_scroll_devices > max_scroll) g_scroll_devices = max_scroll;
    }
    ...
    for (int i = 0; i < g_scan.device_count; i++) {
        ...
        if (iy + item_h < list_y+26 || iy > list_y + list_h) continue;
        ...
        Color bg = is_sel ? COLOR_SELECTED : (hover ? COLOR_PANEL_HOVER : (Color){0,0,0,0});
        if (is_sel || hover) DrawRectangleRounded(item_r, 0.1f, 6, bg);
        if (is_sel) DrawRectangle(item_r.x, item_r.y+4, 3, item_r.height-8, COLOR_ACCENT);

        DrawTextC(d->ip, item_r.x+14, item_r.y+11, 13, is_sel ? COLOR_ACCENT : COLOR_TEXT);

        if (hover && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            if (is_sel) { g_selected_device_ip[0] = '\0'; g_selected_packet_num = -1; }
            else { strncpy(g_selected_device_ip, d->ip, MAX_IP_LEN-1); g_device_detail_subtab = 0; g_scroll_device_detail = 0; g_selected_packet_num = -1; }
        }
    }
    EndScissorMode();

    draw_custom_scrollbar(12+list_w-10, list_y+26, 10, list_h-30, g_scan.device_count * item_h, &g_scroll_devices);
...
static void draw_right_panel_logs(int rx, int ry, int rw, int rh) {
    ...
    char sc[32]; snprintf(sc, sizeof(sc), "#%d", g_scan.scan_count);
    ...
    for (int i = 0; i < g_scanlog.count; i++) {
        int idx = (g_scanlog.write_idx - g_scanlog.count + i + MAX_SCAN_LOG_LINES*2) % MAX_SCAN_LOG_LINES;
        if (idx < 0 || idx >= MAX_SCAN_LOG_LINES) continue;
        Color lc = str_contains(g_scanlog.lines[idx], "hata") ? COLOR_RED :
                   str_contains(g_scanlog.lines[idx], "bulundu") ? COLOR_GREEN : COLOR_TEXT_DIM;
...
static void draw_right_panel_device(int rx, int ry, int rw, int rh) {
    ...
    if (xhov) DrawRectangleRounded(xbtn, 0.3f, 4, (Color){255,255,255,12});
    DrawTextC("X", rx+rw-22, ry+9, 12, xhov ? COLOR_RED : COLOR_TEXT_DIM);
    if (xhov && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) { g_selected_device_ip[0] = '\0'; g_selected_packet_num = -1; return; }

    DrawRectangle(rx+4, ry+28, rw-8, 1, COLOR_BORDER);

    /* Tüm paketleri al, bu cihaza ait olanları filtrele (Stack overflow'u önlemek için static yapıldı) */
    ...
    int dpc = 0;
    for (int i = 0; i < c; i++) {
        if (strcmp(all_packets[i].src_ip, g_selected_device_ip) == 0 || strcmp(all_packets[i].dst_ip, g_selected_device_ip) == 0 ||
            strcmp(all_packets[i].src_mac, g_selected_device_ip) == 0 || strcmp(all_packets[i].dst_mac, g_selected_device_ip) == 0) {
            if (dpc < 1024) dev_packets[dpc++] = all_packets[i];
        }
    }
    ...
    if (g_selected_packet_num != -1) {
        if (GuiButton((Rectangle){rx+rw-80, py-2, 70, 18}, "Geri Don")) { g_selected_packet_num = -1; g_scroll_pdu_detail = 0; }
    }
    ...
    if (g_selected_packet_num == -1) {
        ...
        if (dpc == 0) {
            DrawTextC("Henuz cihaz trafigi yakalanmadi.", rx+8, py+20, 11, COLOR_TEXT_DIM);
        } else {
            int lh = rh - (py - ry) - 4;
            float ms = dpc * 18 - lh;
            if (ms < 0) ms = 0;
            ...
            if (CheckCollisionPointRec(GetMousePosition(), fa)) {
                ...
                if (g_scroll_pcap_flows < 0) g_scroll_pcap_flows = 0;
                if (g_scroll_pcap_flows > ms) g_scroll_pcap_flows = ms;
            }
            BeginScissorModeScaled(rx, py, rw, lh);
            for (int i = 0; i < dpc; i++) {
                int iy = py + i * 18 - (int)g_scroll_pcap_flows;
                if (iy + 18 < py || iy > py + lh) continue;
                PacketRecord *p = &dev_packets[i];
                
                Rectangle pr = {rx+4, iy, rw-24, 17};
                int hover = CheckCollisionPointRec(GetMousePosition(), pr);
                Color rbg = hover ? COLOR_PANEL_HOVER : ((i%2==0) ? (Color){12,16,28,255} : COLOR_SURFACE);
                DrawRectangleRec(pr, rbg);
                
                Color pc = strcmp(p->protocol,"TCP")==0 ? COLOR_CYAN : (strcmp(p->protocol,"UDP")==0 ? COLOR_ACCENT2 : COLOR_TEXT);
                if (strcmp(p->protocol, "DNS")==0 || strcmp(p->protocol, "HTTP")==0 || strcmp(p->protocol, "TLS")==0) pc = COLOR_GREEN;
                if (strcmp(p->protocol, "ARP")==0) pc = COLOR_AMBER;
                if (strcmp(p->protocol, "ICMP")==0) pc = COLOR_RED;
                ...
                DrawTextC(p->src_ip[0] ? p->src_ip : p->src_mac, rx+90, iy+3, 9, COLOR_TEXT);
                DrawTextC(p->dst_ip[0] ? p->dst_ip : p->dst_mac, rx+190, iy+3, 9, COLOR_TEXT);
                
                DrawTextC(p->protocol, rx+290, iy+3, 9, pc);
                
                char infoshort[128];
                strncpy(infoshort, p->info, 127); infoshort[127]='\0';
                for (int c=0; c<127 && infoshort[c]; c++) if (infoshort[c]=='\n'||infoshort[c]=='\r') infoshort[c]=' ';
            ...
            EndScissorMode();
            draw_custom_scrollbar(rx+rw-10, py, 10, lh, dpc * 18, &g_scroll_pcap_flows);
        }
    } else {
        /* Detaylı PDU Görünümü (Wireshark tarzı) */
        PacketRecord *sel_p = NULL;
        for (int i = 0; i < dpc; i++) {
            if (dev_packets[i].packet_number == g_selected_packet_num) { sel_p = &dev_packets[i]; break; }
        }
        
        if (!sel_p) {
            g_selected_packet_num = -1; return;
        }
        ...
        for (int i=0; i<sel_p->layer_count; i++) {
            ...
            if (g_selected_layer == i) {
                // Fields kaç satır?
                int lines = 1;
                for (int c=0; sel_p->layers[i].fields[c]; c++) if (sel_p->layers[i].fields[c]=='\n') lines++;
                d_y += lines * 14 + 10;
            }
        }
        ...
        if (CheckCollisionPointRec(GetMousePosition(), da)) {
            ...
            if (g_scroll_pdu_detail < 0) g_scroll_pdu_detail = 0;
            if (g_scroll_pdu_detail > ms) g_scroll_pdu_detail = ms;
        }
        ...
        for (int i = 0; i < sel_p->layer_count; i++) {
            ...
            char lhead[512];
            snprintf(lhead, sizeof(lhead), "%s %s: %s", g_selected_layer==i ? "v" : ">", l->name, l->summary);
            DrawTextC(lhead, rx+14, cy+5, 11, COLOR_ACCENT);
            
            if (hover && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                if (g_selected_layer == i) g_selected_layer = -1;
                else g_selected_layer = i;
            }
            cy += 24;
            
            if (g_selected_layer == i) {
                char tmpf[1024];
                strncpy(tmpf, l->fields, 1023); tmpf[1023]='\0';
        ...
        for (int r=0; r < sel_p->raw_len; r+=16) {
            ...
            for (int c=0; c<16; c++) {
                if (r+c < sel_p->raw_len) {
                    char hb[8]; snprintf(hb, sizeof(hb), "%02X ", sel_p->raw_data[r+c]);
                    strcat(hexp, hb);
...
static void draw_panel_security(int W, int H) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i == g_security_subtab) {
            ...
            if (sh) DrawRectangleRounded(sb, 0.4f, 4, (Color){255,255,255,6});
            DrawTextC(stabs[i], stx+10, y0+5, 12, sh ? COLOR_TEXT : COLOR_TEXT_SEC);
        }
        if (sh && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) { g_security_subtab = i; g_scroll_alerts = 0; g_scroll_rawlog = 0; }
        stx += sw + 4;
    }
    ...
    if (g_security_subtab == 0) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            draw_stat_card((Rectangle){12 + i*cw, py, cw-4, 50}, cats[i].label, nbuf, cats[i].color, NULL);
        }

        int panel_y = py + 58;
        DrawRoundedPanel((Rectangle){12, panel_y, W-24, H-panel_y-8}, COLOR_PANEL, COLOR_BORDER);
        char title[64];
        snprintf(title, sizeof(title), "Guvenlik Uyarilari (%d)", g_analysis.alert_count);
        DrawTextC(title, 24, panel_y+8, 12, COLOR_ACCENT);

        Rectangle clr_btn = {W-110, panel_y+5, 80, 20};
        if (GuiButton(clr_btn, "Temizle")) { analyzer_clear_alerts(); analyzer_run(&g_analysis); }
        ...
        for (int i = 0; i < g_analysis.alert_count; i++) {
            ...
            Color bg = str_contains(a->severity, "KRITIK") ? (Color){40,8,8,255} :
                       str_contains(a->severity, "YUKSEK") ? (Color){35,18,6,255} : COLOR_SURFACE;
            ...
            Color sc = str_contains(a->severity, "KRITIK") ? COLOR_RED :
                       ...
                       str_contains(a->severity, "ORTA")   ? (Color){234,179,8,255} : COLOR_GREEN;
            DrawRectangle(ir.x, ir.y, 3, ir.height, sc);

            int slw = MeasureText(a->severity, 8) + 8;
            DrawRectangleRounded((Rectangle){ir.x+8, ir.y+5, slw, 12}, 0.5f, 4, (Color){sc.r, sc.g, sc.b, 50});
        ...
        /* === Sistem Loglari === */
        DrawRoundedPanel((Rectangle){12, py, W-24, H-py-8}, COLOR_PANEL, COLOR_BORDER);
        DrawTextC("Sistem Log Analizi", 24, py+8, 12, COLOR_ACCENT);

        const char *sources[] = {"journal", "httpd_access", "httpd_error", "mariadb", "dmesg"};
        const char *labels[]  = {"Journal", "HTTP", "Hatalar", "DB", "Kernel"};
        int bx = W - 420;
        for (int i = 0; i < 5; i++) {
            Rectangle b = {bx, py+5, 76, 20};
            int active = (strcmp(g_raw_log_source, sources[i]) == 0);
            if (active) DrawRectangleRounded(b, 0.4f, 4, COLOR_SELECTED);
            int hover = CheckCollisionPointRec(GetMousePosition(), b);
            if (hover && !active) DrawRectangleRounded(b, 0.4f, 4, (Color){255,255,255,6});
            DrawTextC(labels[i], bx+6, py+9, 10, active ? COLOR_ACCENT : (hover ? COLOR_TEXT : COLOR_TEXT_SEC));
            if (hover && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
                strncpy(g_raw_log_source, sources[i], sizeof(g_raw_log_source));
                g_raw_log_count = analyzer_read_raw_log(g_raw_log_source, g_raw_log_lines, 200);
                g_scroll_rawlog = 0;
            }
            bx += 82;
        }
        ...
        if (CheckCollisionPointRec(GetMousePosition(), la)) {
            ...
            if (g_scroll_rawlog < 0) g_scroll_rawlog = 0;
            float mx = g_raw_log_count * 13 - la.height;
            if (mx < 0) mx = 0;
            if (g_scroll_rawlog > mx) g_scroll_rawlog = mx;
        }
        ...
        if (g_raw_log_count == 0) {
            DrawTextC("Log bulunamadi.", la.x+20, la.y+la.height/2, 12, COLOR_TEXT_SEC);
        }

        BeginScissorModeScaled(la.x, la.y, la.width, la.height);
        for (int i = 0; i < g_raw_log_count; i++) {
            int ly = la.y + 4 + i*13 - (int)g_scroll_rawlog;
            if (ly+13 < la.y || ly > la.y+la.height) continue;
            Color lcolor = str_contains_ci(g_raw_log_lines[i], "error") || str_contains_ci(g_raw_log_lines[i], "crit") ? COLOR_RED :
                           str_contains_ci(g_raw_log_lines[i], "warn") ? COLOR_AMBER :
                           str_contains_ci(g_raw_log_lines[i], "failed") || str_contains_ci(g_raw_log_lines[i], "denied") ? (Color){245,158,100,255} :
                           COLOR_TEXT_DIM;
...
static void draw_panel_tools(int W, int H) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (sh && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) { g_tools_subtab = i; }
        stx += sw + 4;
    }
    ...
    if (g_tools_subtab == 0) {
        ...
        DrawRoundedPanel((Rectangle){12, py, ctrl_w, panel_h}, COLOR_PANEL, COLOR_BORDER);
        DrawTextC("Port Tarayici", 24, py+8, 13, COLOR_ACCENT);

        portscan_get_results(&g_portscan);
        int is_this = (g_bf_target[0] && strcmp(g_portscan.target_ip, g_bf_target) == 0);
        ...
        if (g_bf_target[0]) {
            DrawRectangleRounded((Rectangle){64, cy-2, MeasureText(g_bf_target,10)+10, 14}, 0.4f, 4, COLOR_SELECTED);
        ...
        for (int i = 0; i < g_scan.device_count; i++) {
            ...
            if (sel) DrawRectangleRounded(db, 0.2f, 4, COLOR_SELECTED);
            else if (hov) DrawRectangleRounded(db, 0.2f, 4, COLOR_PANEL_HOVER);
            if (sel) DrawRectangle(db.x, db.y+3, 3, db.height-6, COLOR_ACCENT);
            DrawTextC(g_scan.devices[i].ip, db.x+10, db.y+4, 10, sel ? COLOR_ACCENT : COLOR_TEXT);
            if (hov && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
                strncpy(g_bf_target, g_scan.devices[i].ip, MAX_IP_LEN-1);
        }
        EndScissorMode();
        draw_custom_scrollbar(ip_area.x + ip_area.width - 10, ip_area.y, 10, ip_list_h, g_scan.device_count * item_h, &g_scroll_ps_devices);
        cy += ip_list_h + 8;
        ...
        if (g_bf_target[0] == '\0') {
            ...
            /* Progress bar */
            int target = g_portscan.total_target_ports > 0 ? g_portscan.total_target_ports : 100;
            ...
            cy += 14;
            DrawRectangleRounded((Rectangle){24, cy, pbar_w, 18}, 0.5f, 4, (Color){20,28,48,255});
            if (prog > 0.005f)
                DrawRectangleRounded((Rectangle){24, cy, pbar_w*prog, 18}, 0.5f, 4, COLOR_ACCENT);
            snprintf(buf, sizeof(buf), "%d / %d  (%.0f%%)", g_portscan.total_scanned, target, prog*100);
        ...
        if (is_this && (g_portscan.open_count > 0 || g_portscan.scan_complete)) {
            ...
            DrawTextC(buf, 24, cy, 12, COLOR_GREEN); cy += 16;
            snprintf(buf, sizeof(buf), "Filtrelenmis: %d", g_portscan.filtered_count);
            DrawTextC(buf, 24, cy, 10, COLOR_AMBER); cy += 13;
            snprintf(buf, sizeof(buf), "Taranan: %d / Sure: %.1fs", g_portscan.total_scanned, g_portscan.scan_time_sec);
            DrawTextC(buf, 24, cy, 10, COLOR_TEXT_SEC); cy += 13;
            if (g_portscan.os_guess[0]) {
                snprintf(buf, sizeof(buf), "OS: %s (%d%%)", g_portscan.os_guess, g_portscan.os_confidence);
                DrawTextC(buf, 24, cy, 10, COLOR_CYAN); cy += 13;
            }
        ...
        int rx = 12 + ctrl_w + 8;
        DrawRoundedPanel((Rectangle){rx, py, result_w, panel_h}, COLOR_PANEL, COLOR_BORDER);
        ...
        if (!is_this || (g_portscan.open_count == 0 && !g_portscan.scan_complete)) {
            const char *msg = scanning ? "Tarama devam ediyor..." : "Tarama baslatilmadi.";
            int mw = MeasureText(msg, 12);
            DrawTextC(msg, rx+result_w/2-mw/2, py+panel_h/2, 12, COLOR_TEXT_SEC);
        } else if (g_portscan.open_count == 0 && g_portscan.scan_complete) {
            DrawTextC("Acik port bulunamadi.", rx+result_w/2-70, py+panel_h/2, 12, COLOR_GREEN);
        } else {
            ...
            for (int i = 0; i < g_portscan.open_count; i++) {
                total_h += 22;
                if (g_ps_selected_vuln_port == g_portscan.ports[i].port && g_portscan.ports[i].vuln_count > 0)
            ...
            if (CheckCollisionPointRec(GetMousePosition(), la)) {
                ...
                if (g_scroll_tool_ports < 0) g_scroll_tool_ports = 0;
                if (g_scroll_tool_ports > ms) g_scroll_tool_ports = ms;
            }
            ...
            for (int i = 0; i < g_portscan.open_count; i++) {
                ...
                if (ry2 > list_y0 + list_h) break;

                int row_h = 22;
                int expanded = (g_ps_selected_vuln_port == pr->port && pr->vuln_count > 0);
                if (expanded) row_h += pr->vuln_count * 16 + 8;
                ...
                if (ry2 + row_h >= list_y0) {
                    ...
                    if (pr->vuln_count > 0) {
                        ...
                        for (int v = 0; v < pr->vuln_count; v++)
                            if (strcmp(pr->vulns[v].severity, "CRITICAL") == 0) { vc = COLOR_RED; break; }
                    ...
                    const char *status_str = pr->status == PORT_OPEN ? "open" : pr->status == PORT_FILTERED ? "filtered" : "open|flt";
                    Color stc = pr->status == PORT_OPEN ? COLOR_GREEN : COLOR_AMBER;
                    DrawTextC(status_str, rx+60, ry2+5, 10, stc);

                    int danger = (pr->port==4444||pr->port==5555||pr->port==31337||pr->port==6667);
                    DrawTextC(pr->service, rx+120, ry2+5, 10, danger ? COLOR_RED : COLOR_CYAN);
                    ...
                    if (pr->vuln_count > 0) {
                        ...
                        for (int v = 0; v < pr->vuln_count; v++)
                            if (strcmp(pr->vulns[v].severity, "CRITICAL") == 0) { bc = COLOR_RED; break; }
                        DrawRectangleRounded((Rectangle){rx+result_w-144, ry2+4, bww, 14}, 0.5f, 4, (Color){bc.r,bc.g,bc.b,60});
                    ...
                    if (hov && IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && pr->vuln_count > 0) {
                        if (g_ps_selected_vuln_port == pr->port) g_ps_selected_vuln_port = -1;
                        else g_ps_selected_vuln_port = pr->port;
                    }
                    ...
                    if (expanded) {
                        ...
                        for (int v = 0; v < pr->vuln_count; v++) {
                            ...
                            Color sc2 = strcmp(vn->severity,"CRITICAL")==0 ? COLOR_RED : strcmp(vn->severity,"HIGH")==0 ? COLOR_AMBER : (Color){234,179,8,255};
                            DrawRectangle(rx+20, vy, result_w-28, 15, (Color){sc2.r/6,sc2.g/6,sc2.b/6,255});
                            DrawRectangle(rx+20, vy, 2, 15, sc2);

                            int svw = MeasureText(vn->severity, 8) + 6;
                            DrawRectangleRounded((Rectangle){rx+26, vy+2, svw, 11}, 0.5f, 4, (Color){sc2.r,sc2.g,sc2.b,40});
                            DrawTextC(vn->severity, rx+29, vy+3, 8, sc2);
                            DrawTextC(vn->cve_id, rx+30+svw, vy+3, 8, COLOR_TEXT);

                            char desc_short[80];
                            strncpy(desc_short, vn->description, 79); desc_short[79]='\0';
                            DrawTextC(desc_short, rx+30+svw+MeasureText(vn->cve_id,8)+8, vy+3, 8, COLOR_TEXT_DIM);
                            vy += 16;
                        }
                    }
                }
            ...
            EndScissorMode();
            draw_custom_scrollbar(rx+result_w-10, list_y0, 10, list_h, total_h, &g_scroll_tool_ports);
        }
    } else {
        /* === Brute Force === */
        ...
        snprintf(buf, sizeof(buf), "%d/%d", g_bf_state.tested_combos, g_bf_state.total_combos);
        ...
        draw_stat_card((Rectangle){12+cw, py, cw-4, 66}, "Bulunan",
                       buf, g_bf_state.result_count > 0 ? COLOR_RED : COLOR_GREEN,
                       g_bf_state.result_count > 0 ? "Zayif Kimlik!" : "Temiz");

        snprintf(buf, sizeof(buf), "%.1fs", g_bf_state.elapsed_sec);
        draw_stat_card((Rectangle){12+cw*2, py, cw-4, 66}, "Sure", buf, COLOR_ACCENT2, "Gecen sure");

        snprintf(buf, sizeof(buf), "%d", g_bf_state.thread_count);
        draw_stat_card((Rectangle){12+cw*3, py, cw-4, 66}, "Thread", buf, COLOR_TEXT, "Paralel islem");
        ...
        if (CheckCollisionPointRec(GetMousePosition(), ip_area)) {
            ...
            if (g_scroll_bf_devices < 0) g_scroll_bf_devices = 0;
            if (g_scroll_bf_devices > ip_max_scroll) g_scroll_bf_devices = ip_max_scroll;
        }
        BeginScissorModeScaled(ip_area.x, ip_area.y, ip_area.width, ip_area.height);
        for (int i = 0; i < g_scan.device_count; i++) {
        ...
        EndScissorMode();
        draw_custom_scrollbar(ip_area.x + ip_area.width - 10, ip_area.y, 10, ip_list_h, g_scan.device_count * item_h, &g_scroll_bf_devices);
        cy += ip_list_h + 8;
        ...
        cy += 16;
        /* BF_PROTO_FTP=0, HTTP_POST=1, HTTP_GET=2, MYSQL=3, TELNET=4, SSH=5, SMB=6, RDP=7 */
        const char *proto_labels[] = {"FTP","HTTP-P","HTTP-G","MySQL","Telnet","SSH","SMB","RDP"};
        for (int i = 0; i < 8; i++) {
            int bx = 24 + (i % 4) * 76;
            int by = cy + (i / 4) * 24;
            ...
            Color bg = sel ? COLOR_SELECTED : (hover ? COLOR_PANEL_HOVER : (Color){20,28,48,255});
            DrawRectangleRounded(pb, 0.3f, 4, bg);
            if (sel) DrawRectangle(bx, by+18, 72, 2, COLOR_ACCENT);
            DrawTextC(proto_labels[i], bx+4, by+5, 9, sel ? COLOR_ACCENT : COLOR_TEXT_DIM);
            if (hover && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) g_bf_proto_sel = i;
        }
        cy += 54;
        ...
        if (hover_slider && IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
            ...
            if (g_bf_threads < 1) g_bf_threads = 1;
            if (g_bf_threads > 50) g_bf_threads = 50;
        }
        float pct = (g_bf_threads - 1) / 49.0f;
        DrawRectangleRounded((Rectangle){24, cy, ctrl_w-40, 12}, 0.5f, 4, (Color){20,28,48,255});
        DrawRectangleRounded((Rectangle){24, cy, (ctrl_w-40)*pct, 12}, 0.5f, 4, COLOR_ACCENT);
        ...
        if (!g_bf_state.is_running) {
            if (GuiButton(btn, "Saldiriyi Baslat")) {
                if (g_bf_target[0] != '\0') {
                    bf_start(g_bf_target, g_bf_port_override, (BfProtocol)g_bf_proto_sel, g_bf_threads);
                } else {
            ...
            if (GuiButton(btn, "Saldiriyi Durdur")) bf_stop();
        }
        ...
        if (g_bf_state.target_ip[0]) {
            snprintf(buf, sizeof(buf), "%s:%d [%s]", g_bf_state.target_ip, g_bf_state.port,
                     bf_proto_name(g_bf_state.protocol));
        ...
        if (g_bf_state.result_count == 0) {
            ...
            if (CheckCollisionPointRec(GetMousePosition(), res_area)) {
                ...
                if (g_scroll_bf_results < 0) g_scroll_bf_results = 0;
                if (g_scroll_bf_results > max_s) g_scroll_bf_results = max_s;
            }
            ...
            for (int i = 0; i < g_bf_state.result_count; i++) {
                ...
                if (iy + item_h < list_start || iy > list_start + list_h) continue;
                ...
                DrawTextC(cr->password, rx+result_w/2-40, iy+4, 12, COLOR_AMBER);

                snprintf(buf, sizeof(buf), "%.0fms", cr->time_ms);
                DrawTextC(buf,          rx+result_w-80,   iy+8, 10, COLOR_TEXT_DIM);

                /* TEHLIKE badge */
                DrawRectangleRounded((Rectangle){rx+36, iy+18, 50, 12}, 0.5f, 4, (Color){180,30,30,180});
...
void gui_init(int width, int height) {
    // Pencereyi oluşturmadan önce tam ekran/maximized bayraklarını ekleyelim
    SetConfigFlags(FLAG_WINDOW_RESIZABLE | FLAG_WINDOW_MAXIMIZED | FLAG_MSAA_4X_HINT);
    
    // Önce varsayılan veya verilen boyutla başlat
    InitWindow(width, height, "Akilli Sehir Guvenlik Merkezi");
    
    // Başladıktan sonra PC'nin gerçek (kendi) ekran çözünürlüğünü alıp eşitle
...
void gui_cleanup(void) {
    if (g_custom_font.texture.id > 0) UnloadFont(g_custom_font);
    CloseWindow();
}
...
void gui_draw(void) {
    ...
    /* Ölçek hesaplama */
    float scale = fminf((float)W / V_WIDTH, (float)H / V_HEIGHT);
    g_camera_scale = scale;
    g_camera_offset.x = (W - (V_WIDTH * scale)) * 0.5f;
    g_camera_offset.y = (H - (V_HEIGHT * scale)) * 0.5f;
    
    /* Mouse pozisyonunu sanal çözünürlüğe ayarla */
    Vector2 mouse = GetMousePosition();
    Vector2 virtualMouse = { 0 };
    virtualMouse.x = (mouse.x - (W - (V_WIDTH * scale)) * 0.5f) / scale;
    virtualMouse.y = (mouse.y - (H - (V_HEIGHT * scale)) * 0.5f) / scale;

    /* clamp */
    if (virtualMouse.x < 0) virtualMouse.x = 0;
    if (virtualMouse.x > V_WIDTH) virtualMouse.x = V_WIDTH;
    if (virtualMouse.y < 0) virtualMouse.y = 0;
    if (virtualMouse.y > V_HEIGHT) virtualMouse.y = V_HEIGHT;

    SetMouseOffset(-(W - (V_WIDTH * scale)) * 0.5f, -(H - (V_HEIGHT * scale)) * 0.5f);
    SetMouseScale(1.0f / scale, 1.0f / scale);

    /* Native ekrana çizim yap (Camera2D ile Vektörel) */
    BeginDrawing();
    ClearBackground(BLACK); // Kenar boşlukları siyah

    Camera2D camera = { 0 };
    camera.offset = (Vector2){ (W - (V_WIDTH * scale)) * 0.5f, (H - (V_HEIGHT * scale)) * 0.5f };
    ...
    // İç uygulamanın arka planı
    DrawRectangle(0, 0, V_WIDTH, V_HEIGHT, COLOR_BG);

    draw_header(V_WIDTH);
    draw_tabs(V_WIDTH);

    switch (g_active_tab) {
        case TAB_DASHBOARD: draw_panel_dashboard(V_WIDTH, V_HEIGHT); break;
        case TAB_SECURITY:  draw_panel_security(V_WIDTH, V_HEIGHT); break;
        case TAB_TOOLS:     draw_panel_tools(V_WIDTH, V_HEIGHT); break;
        default: break;
    }
